...
class QPooling2D(Function,
    ...
    for s_h in stride_range(h_i, h_o):
      for s_w in stride_range(w_i, w_o):
        ...
        for p_h in range(1, 7):
          for _t in QPooling2D.PoolingType:
            configurations.append({
              cls.arg_POOLING_HEIGHT: p_h,
              cls.arg_POOLING_WIDTH: p_h,
              cls.arg_STRIDE_HEIGHT: s_h,
              cls.arg_STRIDE_WIDTH: s_w,
              cls.arg_PADDING: QPooling2D.Padding.SAME.value,
          ...
          for s_h in stride_range(h_i - p_h + 1, h_o):
            ...
            for _t in QPooling2D.PoolingType:
              configurations.append({
                ...
                cls.arg_STRIDE_HEIGHT: s_h,
                cls.arg_STRIDE_WIDTH: s_w,
                cls.arg_PADDING: QPooling2D.Padding.VALID.value,
                cls.arg_POOLING_TYPE: _t.value,
              })
  ...
  def generateParameters(cls,
    ...
    configurations = QPooling2D.configurations(h_i=h_i, h_o=h_o, w_i=w_i, w_o=w_o)
  ...
  def __init__(self, **kwargs):
    super(QPooling2D, self).__init__(**kwargs)
  ...
  def mutate(self, prob):
    ...
    if random() < prob:
      ...
      result.attr = {**result.attr,
                     **choice(QPooling2D.configurations(
  ...
  def flops_per_sample(self):
    out_shape = self.attr[self.arg_OUT_NAMED_TYPE_SHAPES][IOLabel.POOLING2D_OUT].shape
    return self.attr[self.arg_POOLING_WIDTH] * self.attr[self.arg_POOLING_HEIGHT] * \
           out_shape[DimNames.CHANNEL] * out_shape[DimNames.WIDTH] * out_shape[DimNames.HEIGHT]
